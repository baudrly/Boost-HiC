...
# input file
bedfilename = (
    "test_dataset/mouse10kb_fend.bed"
)
matrixfilename = (
    "test_dataset/chr16ES_10000.matrix"
)
Operation = "Sample"
repositoryout = (
    "test"
...
def BoostHiC(amat):
    ...
    FFmat = np.power(
        HiCutils.fastFloyd(1 / np.power(normmat.copy(), alpha)), -1 / alpha
    )  # to dist, FF, to contact in one line
...
def Sample(amat, repositoryout):
    ...
    for j in percentofsample:
        ...
        fh5 = h5py.File(
            repositoryout + "inputmat_sampleat_" + str(j) + "_percent.hdf5", "w"
        )
        fh5["data"] = chrmat
        fh5.close()
